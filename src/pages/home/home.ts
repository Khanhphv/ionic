...
export class HomePage {
  ...
  image = 'https://cdnmedia.thethaovanhoa.vn/Upload/leenEplQKY7jsunvYUYgg/files/2018/07/he1.jpg';
  ...
  ngAfterViewInit() {
    this.platform.ready().then((readySource) => {
      this.dodai = this.platform.width()
    });
  }
  ...
  getImg(image) {
    if (image) {
      ...
      source.onload = () => {
        ...
        context.drawImage(source, 0, 0, canvas.width, canvas,canvas.height);
  ...
  downScaleCanvas(cv, scale) {
    ...
    var resCV = this.canvasOutPut.nativeElement
    resCV.width = tw;
    resCV.height = th;
