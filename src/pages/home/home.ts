...
export class HomePage {
  ...
  getPicture(index) {
    ...
    this.camera.getPicture(options).then((imageData) => {
      ...
      this.selectedImage = 'data:image/jpeg;base64,' + imageData;
      // this.recognizeImage()
      // Caman('#processImg', function () {
      //   this.brightness(0);
      //   this.contrast(9);
      //   this.render(function () {
      //     this.selectedImage = this.toBase64()
      //     // alert(this.selectedImage)
      //     this.base64ToGallery.base64ToGallery(this.toBase64).then(
      //       res => alert("success"),
      //       err => alert(err)
      //     );
      //   })
  ...
  canvasImage(index) {
    console.log(this.storage[index].elements.length)
  ...
  saveImageToGallery(index) {
    for (let i = 0; i < this.storage[index].elements.length; i++) {
      ...
      let ctx = $("#canvasOut" + i)[0].getContext('2d');
      ctx.scale(3,3);
      ...
      Tesseract.recognize(ctx, {
        ...
        .then(result => {
          // this.presentLoadingDefault()
          alert(result.text)
          this.textResult.push(result.text)
